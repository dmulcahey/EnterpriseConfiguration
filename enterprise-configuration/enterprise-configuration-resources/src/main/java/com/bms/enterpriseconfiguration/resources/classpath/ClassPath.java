...
public final class ClassPath {
	...
	public ImmutableSet<ClassInfo> getTopLevelClasses() {
		return FluentIterable.from(resources).filter(ClassInfo.class)
				.filter(IS_TOP_LEVEL).toSet();
	}
	 ...
	 */
	public ImmutableSet<ClassInfo> getTopLevelClassesRecursive(
			String packageName) {
		checkNotNull(packageName);
	...
	abstract static class Scanner {
		...
		public final void scan(ClassLoader classloader) throws IOException {
			for (Map.Entry<File, ClassLoader> entry : getClassPathEntries(
		...
		private void scanJar(File file, ClassLoader classloader)
			...
			try {
				for (File path : getClassPathFromManifest(file,
						jarFile.getManifest())) {
		...
		@VisibleForTesting
		@SneakyThrows
		static ImmutableMap<File, ClassLoader> getClassPathEntries(
	...
	static final class DefaultScanner extends Scanner {
		private final SetMultimap<ClassLoader, String> resources = MultimapBuilder
				.hashKeys().linkedHashSetValues().build();
